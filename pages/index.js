 ...
 * Case-insensitive, handles snake_case, camelCase, PascalCase — whatever Cornerstone returns.
 */
...
function extractExecutionSummary(details) {
  return {
    ...
    total_records: Number(deepFind(details, ["total_records", "totalRecords", "total", "record_count", "recordCount"]) ?? 0),
    success_count: Number(deepFind(details, ["success_count", "successCount", "success", "succeeded", "successful"]) ?? 0),
    warning_count: Number(deepFind(details, ["warning_count", "warningCount", "warnings", "warning"]) ?? 0),
    error_count: Number(deepFind(details, ["error_count", "errorCount", "errors", "failed", "failure_count"]) ?? 0),
  };
}
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
                    ...
                    <tbody>
                      {jobs.map((job) => {
                        ...
                        // Collapsed view: only Job ID + Label populated, other cells blank/em-dash.
                        // Expanded view: full import row populated.
                        return (
                          <>
                            <tr
                              key={`row-${job.job_id}`}
